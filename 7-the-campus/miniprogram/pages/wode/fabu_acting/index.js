...
Page({
  ...
  onLoad: function (options) {
    ...
    db.collection("acting").orderBy('time', 'desc').get({
      success: res => {
        this.setData({
          acting_id: res.data
        })
  ...
  onQuery: function () {
    ...
    db.collection('acting').where({
      ...
      success: res => {
        ...
        console.log('[数据库] [查询记录] 成功， 他的值为：', res.data.acting_id)
        this.mation()
      },
      fail: err => {
        wx.showToast({
  ...
  mation: function (e) {
    var acting = this.data.acting
    console.log("acting的值为", acting)
    if (acting.length > 0) {
      this.setData({
        information: true
      })
  ...
  onRemove: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          //点击确认
          db.collection("acting").doc(id).remove({
            success: res => {
              wx.showToast({
                title: '删除成功请刷新',
              })
