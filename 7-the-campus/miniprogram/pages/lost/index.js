...
Page({
  ...
  queryPageOne: function () {
    ...
    //lost
    var res = that.data.lost;
    // 查询当前用户所有的 counters
    db.collection('lost').limit(pageSize).orderBy('time', 'desc').get({
      success: res => {

        //追加数据
        // this.data.lost.unshift(res.data);
        this.setData({
          // lost: JSON.stringify(res.data, null, 2)
          lost: this.data.lost.concat(res.data),
          page: pageNum + 1
        })
  ...
  queryByPage: function () {
    ...
    var res = that.data.lost;
    // 查询当前用户所有的 counters
    db.collection('lost').skip((pageNum - 1) * pageSize).limit(pageSize).orderBy('time', 'desc').get({
      success: res => {
