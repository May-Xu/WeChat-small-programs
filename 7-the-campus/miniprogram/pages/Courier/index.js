...
Page({
  ...
  queryPageOne: function () {
    ...
    //acting
    var res = that.data.acting;
    // 查询当前用户所有的 counters
    db.collection('acting').limit(pageSize).orderBy('time', 'desc').get({
      success: res => {

        //追加数据
        // this.data.acting.unshift(res.data);
        this.setData({
          // acting: JSON.stringify(res.data, null, 2)
          acting: this.data.acting.concat(res.data),
          page: pageNum + 1
        })
  ...
  queryByPage: function () {
    ...
    var res = that.data.acting;
    // 查询当前用户所有的 counters
    db.collection('acting').skip((pageNum - 1) * pageSize).limit(pageSize).orderBy('time', 'desc').get({
      success: res => {
