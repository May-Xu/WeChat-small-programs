...
Page({
  ...
  queryPageOne: function () {
    ...
    //olds
    var res = that.data.olds;
    // 查询当前用户所有的 counters
    db.collection('olds').limit(pageSize).orderBy('time', 'desc').get({
      success: res => {

        //追加数据
        // this.data.olds.unshift(res.data);
        this.setData({
          // olds: JSON.stringify(res.data, null, 2)
          olds: this.data.olds.concat(res.data),
          page: pageNum + 1
        })
  ...
  queryByPage: function () {
    ...
    var res = that.data.olds;
    // 查询当前用户所有的 counters
    db.collection('olds').skip((pageNum - 1) * pageSize).limit(pageSize).orderBy('time', 'desc').get({
      success: res => {
