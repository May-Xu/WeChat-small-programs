...
Page({
  ...
  queryPageOne: function () {
    ...
    //find
    var res = that.data.find;
    // 查询当前用户所有的 counters
    db.collection('find').limit(pageSize).orderBy('time', 'desc').get({
      success: res => {

        //追加数据
        // this.data.find.unshift(res.data);
        this.setData({
          // find: JSON.stringify(res.data, null, 2)
          find: this.data.find.concat(res.data),
          page: pageNum + 1
        })
  ...
  queryByPage: function () {
    ...
    var res = that.data.find;
    // 查询当前用户所有的 counters
    db.collection('find').skip((pageNum - 1) * pageSize).limit(pageSize).orderBy('time', 'desc').get({
      success: res => {
